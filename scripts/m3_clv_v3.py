# =============================================================================
# M3 CLV V3 — Cartier QTEM Data Challenge
# Script B: Full CLV model — V3 improvements over V2
#
# Changes vs V2:
# - Phase 3 : Time-decay sample weights for classifier (recover Recall@10%)
# - Phase 4 : n_estimators=2000, learning_rate=0.02 (fix regressor underfitting)
# - Phase 9c: Permutation feature names fixed — root cause resolved in Phase 2
#             (perm_feat_names always derived from X_train_df post-imputer)
# - Phase 11b: Bootstrap CI for Spearman computed on positives-only subsample
#              (consistent with the point estimate)
# - Phase 5 : Removed duplicate CHECK 5 print
# - All outputs renamed to *_v3 (no collision with V2 artifacts)
#
# Run: python scripts/m3_clv_v3.py
# =============================================================================
...
def categorize_feature(col):
    if any(col.startswith(p) for p in ["HAS_ALL_", "NTOTAL_ALL", "NDISTINCT_ALL",
                                        "DIVERSITY_ALL", "N_TOTAL_ALL", "N_DISTINCT_ALL"]):
        return "ALL_* (V2 parsed)"
    if any(col.startswith(p) for p in ["SPEND_PER", "TREND_X", "HE_PRICE",
                                        "TRS_FREQ", "RECENT_HIST", "TRS_FREQUENCY",
                                        "SPEND_TREND"]):
        return "Interaction (V2)"
    if any(col.startswith(p) for p in ["NCRC", "HAS_CRC", "AVG_DURATION",
                                        "HAS_CLIENTELING", "N_CRC"]):
        return "CRC (V2)"
    ...
    rfm_kws = ["NTRANSACTIONS", "TOTALSPEND", "AVGSPEND", "MAXSINGLE",
               ...
               "RECENCY_DAYS", "TO_AVG_SPREAD", "AVG_DAYS_BETWEEN"]
...
print("""
  CLV_SCORE_i = P(active_i | X)  x  E[spend_i | active, X]
    ...
    P(active)  — XGBoost classifier on BINARY_TARGET_5Y
                 ...
                 Time-decay sample weights applied (V3 improvement)
                 Early stopping monitored on 2015 snapshot (val fold)

    E[spend]   — XGBoost regressor on LOG_TARGET_5Y (positives only)
                 Back-transformed via expm1()  [no smearing — tested in Script A]
                 n_estimators=2000, lr=0.02 (V3: fixes V2 underfitting at 999 trees)
...
def _resolve_targets(df):
    ...
    if "TARGET_3Y" in df.columns:
        ...
        df["LOG_TARGET_5Y"]    = (df["LOG_TARGET_3Y"]    if "LOG_TARGET_3Y"    in df.columns
                                  else np.log1p(df["TARGET_3Y"]))
        df["BINARY_TARGET_5Y"] = (df["BINARY_TARGET_3Y"] if "BINARY_TARGET_3Y" in df.columns
                                  else (df["TARGET_3Y"] > 0).astype(int))
        return
    raise KeyError(
...
# ── Feature sanitation: drop all-NaN columns before any encoding ─────────────
# AVG_DURATION_SNAP (and any future dead features) are 100% missing in train.
# SimpleImputer silently drops them, causing a name/matrix mismatch.
# We remove them explicitly here so the pipeline stays fully consistent.
all_nan_feats = [c for c in FEAT_COLS if train[c].isna().all()]
if all_nan_feats:
    print(f"  Dropping all-NaN features (zero predictive value): {all_nan_feats}")
    FEAT_COLS = [c for c in FEAT_COLS if c not in all_nan_feats]
else:
    print("  No all-NaN features detected")
...
joblib.dump(enc, os.path.join(MODELSDIR, "ordinal_encoder_clv_v3.pkl"))

# ── Build named DataFrames and deduplicate columns before imputation ─────────
# Root cause of V3 assert error: pandas concat can produce duplicate column
# names if a feature (e.g. SAVINGS_RATE_AT_SNAPSHOT) appears in both
# train[num_cols] and the categorical array due to dtype edge cases, OR if
# the savings rate merge introduced a near-duplicate. We deduplicate
# X_train_df columns explicitly, then derive model_feat_names from the
# actual deduplicated column list — single source of truth for all phases.
...
# Canonical feature name list — derived AFTER dedup, BEFORE imputation
# This is the single source of truth used in all downstream phases.
...
joblib.dump(imputer, os.path.join(MODELSDIR, "imputer_clv_v3.pkl"))

# Hard consistency check — will catch any future column drift immediately
assert len(model_feat_names) == X_train_imp.shape[1], (
    f"Feature name/matrix mismatch after dedup: {len(model_feat_names)} names "
    f"vs {X_train_imp.shape[1]} imputed columns"
)
assert X_train_imp.shape[1] == X_test_imp.shape[1], (
    f"Train/test column mismatch: {X_train_imp.shape[1]} vs {X_test_imp.shape[1]}"
)
print(f"  Feature matrix: {X_train_imp.shape[1]} columns (name list verified)")
...
# =============================================================================
# PHASE 3 — CLASSIFIER  P(TARGET_5Y > 0)
# =============================================================================
sep("PHASE 3 — CLASSIFIER  P(TARGET_5Y > 0)")

# ── V3 improvement: time-decay sample weights ─────────────────────────────
# Give more weight to recent snapshots so the classifier better reflects
# 2018–2021 purchasing patterns, reducing the Recall@10% degradation.
# Weights: 2006→0.3, 2009→0.45, 2012→0.6, 2015→0.8, 2018→1.0
# (val fold 2015 is excluded from training so its weight doesn't matter here)
snap_years_train = train_dates[trn_mask].dt.year.values
decay_map = {2006: 0.30, 2009: 0.45, 2012: 0.60, 2018: 1.00}
# Any year not in map (e.g. 2015 is excluded; other years fall back to 1.0)
sample_weight_clf = np.array([decay_map.get(y, 1.0) for y in snap_years_train],
                              dtype=np.float32)

scale_pos = (1 - y_train_bin[trn_mask].mean()) / y_train_bin[trn_mask].mean()
print(f"  scale_pos_weight: {scale_pos:.2f}")
print(f"  Time-decay weights applied — min: {sample_weight_clf.min():.2f}  "
      f"max: {sample_weight_clf.max():.2f}  mean: {sample_weight_clf.mean():.3f}")
...
joblib.dump(clf_clv, os.path.join(MODELSDIR, "classifier_clv_v3.pkl"))
...
# =============================================================================
# PHASE 4 — REGRESSOR  E[log(TARGET_5Y) | active]
# =============================================================================
...
# ── V3 improvement: n_estimators=2000, learning_rate=0.02 ────────────────
# V2 used 1000 trees / lr=0.03 and the regressor reached the tree limit
# without early stopping triggering (RMSE still declining at tree 999).
# Doubling capacity + lower lr gives the model room to find its true optimum.
...
reg_clv.fit(X_tr_pos, y_tr_pos,
            ...
            verbose=100)
joblib.dump(reg_clv, os.path.join(MODELSDIR, "regressor_clv_v3.pkl"))
print(f"  Trees used (best iteration): {reg_clv.best_iteration}")
if reg_clv.best_iteration >= 1950:
    print("  NOTE: regressor still near tree limit — consider n_estimators=3000 if time allows")
...
# V3 fix: single CHECK 5 print (V2 had a duplicate)
if rev_cap[0.10] >= 0.55:
    ...
    print("  CHECK 5 — Revenue capture recorded — PASS")

# =============================================================================
# PHASE 6 — SEGMENTATION CLV 4-TIER (2021 snapshot)
# =============================================================================
...
res.to_csv(os.path.join(OUTPUTDIR, "clv_segmentation_2021_v3.csv"), index=False)
tier_stats.to_csv(os.path.join(OUTPUTDIR, "clv_tier_summary_2021_v3.csv"))
...
panel_out.to_csv(os.path.join(OUTPUTDIR, "clv_panel_all_snapshots_v3.csv"), index=False)
...
early_df.to_csv(os.path.join(OUTPUTDIR, "clv_early_detection_v3.csv"), index=False)
...
clf_imp.to_csv(os.path.join(OUTPUTDIR, "clf_importance_gain_v3.csv"), index=False)
...
reg_imp.to_csv(os.path.join(OUTPUTDIR, "reg_importance_gain_v3.csv"), index=False)

# ── 9c: Permutation importance on test set (top 15) ─────────────────────────
# V3 fix: perm_feat_names is always model_feat_names (set in Phase 2 and
# verified equal to both X_train_imp and X_test_imp column count).
# No fallback to f0..fN generic names needed.
print("\n  ── 9c: PERMUTATION IMPORTANCE on test set (top 15) ──")
perm_feat_names = model_feat_names   # guaranteed correct — asserted in Phase 2
print(f"  Feature names: {len(perm_feat_names)} (verified == X_test_imp columns)")
...
perm_clf_df.to_csv(os.path.join(OUTPUTDIR, "clf_importance_permutation_v3.csv"), index=False)
...
perm_reg_df.to_csv(os.path.join(OUTPUTDIR, "reg_importance_permutation_v3.csv"), index=False)
...
# V3 fix: Spearman CI computed on positives-only subsample within each
# bootstrap draw — consistent with how the point estimate is computed.
...
for _ in range(N_BOOT):
    ...
    pos_idx = pos_mask_te[idx]          # boolean mask within the bootstrap draw

    # Spearman: positives-only (V3 fix — consistent with point estimate)
    sp_val = np.nan
    if pos_idx.sum() > 1:
        sp_val, _ = spearmanr(y_test_raw[idx][pos_idx],
                               clv_score[idx][pos_idx])

    # PR-AUC: all clients (needs both classes)
    pa_val = np.nan
    if y_test_bin[idx].sum() > 0 and (1 - y_test_bin[idx]).sum() > 0:
...
print(f"  {'Metric':<22} {'Point Est':>11} {'95% CI':>22}")
print(f"  {'-'*55}")
...
# Sanity check: point estimate should lie within CI
for name, pt, lo, hi in [("Spearman", spearman_test, sp_lo, sp_hi),
                          ("PR-AUC",   prauc_test,    pa_lo, pa_hi),
                          ("RevCap",   rc_test[0.10], rc_lo, rc_hi)]:
    if not (lo <= pt <= hi):
        print(f"  WARNING: {name} point estimate {pt:.4f} outside CI [{lo:.4f}, {hi:.4f}]")
...
for feat in profile_features:
    ...
    profile_rows.append({"feature": feat, "VIC_mean": vic_mean,
                          "Dormant_mean": dorm_mean, "VIC_vs_Dormant_ratio": ratio})

profile_df = (pd.DataFrame(profile_rows)
              .sort_values("VIC_vs_Dormant_ratio", ascending=False)
              .reset_index(drop=True))
print(profile_df.to_string(index=False))
profile_df.to_csv(os.path.join(OUTPUTDIR, "vic_vs_dormant_profile_v3.csv"), index=False)

# ── 12b: Spend concentration ─────────────────────────────────────────────────
...
lorenz_df.to_csv(os.path.join(OUTPUTDIR, "lorenz_curve_v3.csv"), index=False)

# ── 12c: Upgrade path (Aspirational → VIC) ──────────────────────────────────
...
upgrade_df.to_csv(os.path.join(OUTPUTDIR, "upgrade_path_vic_v3.csv"), index=False)
...
for feat in profile_features:
    ...
    fv_vals = (test.loc[test["CLIENT_ID"].isin(ids), feat].values
               if "CLIENT_ID" in test.columns else np.array([]))
...
if upgrade_feat_rows:
    ...
    upg_feat_df.to_csv(os.path.join(OUTPUTDIR, "upgrade_path_features_v3.csv"), index=False)
...
dod_checks = [
    ("Classifier trained (pkl)",
     os.path.exists(os.path.join(MODELSDIR,"classifier_clv_v3.pkl")), ""),
    ("Regressor trained (pkl)",
     os.path.exists(os.path.join(MODELSDIR,"regressor_clv_v3.pkl")),  ""),
    ...
    ("Segmentation 2021 saved",
     os.path.exists(os.path.join(OUTPUTDIR,"clv_segmentation_2021_v3.csv")), ""),
    ("Panel all snapshots saved",
     os.path.exists(os.path.join(OUTPUTDIR,"clv_panel_all_snapshots_v3.csv")), ""),
    ("Early detection saved",
     os.path.exists(os.path.join(OUTPUTDIR,"clv_early_detection_v3.csv")), ""),
    ("Permutation importance saved",
     os.path.exists(os.path.join(OUTPUTDIR,"clf_importance_permutation_v3.csv")), ""),
    ("Robustness section complete",  True, "PASS"),
    ("Business insights saved",
     os.path.exists(os.path.join(OUTPUTDIR,"vic_vs_dormant_profile_v3.csv")), ""),
]
...
esito = "M3 V3 COMPLETE" if all_pass else "M3 V3 INCOMPLETE — check FAIL items above"
print(f"\n  RESULT: {esito}")

# ── Final report CSV ─────────────────────────────────────────────────────────
report_rows = [
    ("Model",  "Version",        "V3", "Time-decay clf + 2000-tree reg + perm fix + CI fix"),
    ...
    ("Model",  "Savings rate feature", "YES" if has_sr else "NO",
     "Merged by snapshot year"),
    ("Model",  "Clf sample weights", "Time-decay (2006:0.3 → 2018:1.0)",
     "V3: recover Recall@10% on 2021 test set"),
    ("Model",  "Reg n_estimators", "2000 (lr=0.02)",
     "V3: V2 regressor hit tree limit at 999 — fixed"),
    ...
    ("Bootstrap","Spearman 95% CI",  f"[{sp_lo:.4f} — {sp_hi:.4f}]", "200 samples, positives-only"),
...
(pd.DataFrame(report_rows, columns=["category","metric","value","note"])
   .to_csv(os.path.join(OUTPUTDIR, "m3_clv_v3_report.csv"), index=False))

sep("OUTPUTS GENERATED")
tables = [
    "clv_segmentation_2021_v3.csv",
    "clv_tier_summary_2021_v3.csv",
    "clv_panel_all_snapshots_v3.csv",
    "clv_early_detection_v3.csv",
    "clf_importance_gain_v3.csv",
    "reg_importance_gain_v3.csv",
    "clf_importance_permutation_v3.csv",
    "reg_importance_permutation_v3.csv",
    "vic_vs_dormant_profile_v3.csv",
    "lorenz_curve_v3.csv",
    "upgrade_path_vic_v3.csv",
    "upgrade_path_features_v3.csv",
    "m3_clv_v3_report.csv",
]
models = [
    "classifier_clv_v3.pkl",
    "regressor_clv_v3.pkl",
    "imputer_clv_v3.pkl",
    "ordinal_encoder_clv_v3.pkl",
]
for f in tables:
